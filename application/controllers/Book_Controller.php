...
class Book_Controller extends CI_Controller {
    ...
    public function book_search($books) {
        ...
        if ($books != null) {
            ...
            $this->load->view('Books', array('results' => $books, 'role' => $role,'error'=>'Không tìm thấy sách nào'));
        }
    }
    ...
    public function sort($property=null){
        ...
        // Không search cái khác tham số
        if($property!=null && $property !='title' && $property !='price'){
            //$this->books();
            //return;
        }
        ...
        // Nếu trang xem sách đang là mặc định
        if($this->session->userdata($property)==null){
            // Lưu action vào session, gồm tên thuộc tính và kiểu sắp xếp, mặc định là ASC
            $this->session->set_userdata($property,'ASC');
        }
        else{
            // Nếu đã có action sort thực hiện trước đó, thì tiến hành sort ngược lại
            $this->session->userdata($property) == "ASC" ? 
                    $this->session->set_userdata($property,"DESC") : 
                $this->session->set_userdata($property,"ASC");
        }
        
        $this->do_sort($property);
    }
    
    public function do_sort($property){
        ...
        if ($total_records > 0) {
            ...
            // Lấy dữ liệu của page hiện tại nạp vào param
            $params["results"] = $this->Book_Model->get_current_page_records($limit_per_page, $start_index,true,$property,$this->session->userdata($property));
